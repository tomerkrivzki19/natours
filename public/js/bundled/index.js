...
if (createTourFormSeconed) createTourFormSeconed.addEventListener("submit", (e)=>{
    ...
    for (let file of files)if (file.size > MAX_FILE_SIZE) {
    ...
    document.querySelectorAll(".locations-sub-container").forEach((container)=>{
        const coordinates = [
            parseFloat(container.querySelector('input[name="startLocation.coordinates[0]"]').value) || 1.1,
            parseFloat(container.querySelector('input[name="startLocation.coordinates[1]"]').value) || 1.1
        ];
        const description = container.querySelector('input[name="startLocation.description"]').value;
        const day = container.querySelector('input[name="startLocation.day"]').value;
        locations.push({
            coordinates,
            description,
            day
