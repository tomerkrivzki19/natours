...
const codeAuthentication = document.querySelector(".form--login-code-authentication");
...
if (phoneNumberForm) phoneNumberForm.addEventListener("submit", (e)=>{
    e.preventDefault();
    const formData = new FormData(e.target);
    (0, _login.sendPhoneNumber)(formData);
});
//verify form with code text
if (codeAuthentication) codeAuthentication.addEventListener("submit", (e)=>{
    e.preventDefault();
    const formData = new FormData(e.target);
    ...
    // console.log(code);
    (0, _login.verifyPhoneText)(phoneNumber, code);
});
...
if (singupForm) singupForm.addEventListener("submit", (e)=>{
    ...
    (0, _login.singup)(name, email, password, passwordConfirm, phoneNumber);
...
//confirm email:
if (emailConfirm) emailConfirm.addEventListener("click", (e)=>{
    e.preventDefault();
    const email = emailConfirm.getAttribute("data");
    (0, _updateSettings.sendConfirmEmail)(email);
});
...
if (deleteReviewBtn) deleteReviewBtn.forEach((deleteButton)=>{
    deleteButton.addEventListener("click", (e)=>{
        e.preventDefault();
        const reviewId = deleteButton.getAttribute("data-review-id");
        // console.log(reviewId);
        (0, _sendReview.deleteReview)(reviewId);
    });
});
// Favorites
if (favorite) favorite.addEventListener("click", (e)=>{
    e.preventDefault();
...
if (addLocationBtn) addLocationBtn.addEventListener("click", (e)=>{
    ...
    if (locationContainers.length < 6) {
        ...
        subContainer.innerHTML = `
          <label class="heading-secondary ma-bt-lg" for=" ">${newIndex + 1}</label>
...
if (createTourFormSeconed) createTourFormSeconed.addEventListener("submit", (e)=>{
    ...
    // Append startDates
    document.querySelectorAll(".start-dates-sub-container").forEach((container, index)=>{
        const date = container.querySelector('input[name^="startDates"][name$=".date"]').value;
        formData.append(`startDates[${index}][date]`, date);
    });
    // Append guides
    formData.append("guides[0]", getFormElementValue('select[name="guides[0]"]'));
    formData.append("guides[1]", getFormElementValue('select[name="guides[1]"]'));
    formData.append("guides[2]", getFormElementValue('select[name="guides[2]"]'));
    // Append locations
    document.querySelectorAll(".locations-sub-container").forEach((container, index)=>{
        const coordinates = [
            parseFloat(container.querySelector('input[name="location.coordinates[0]"]').value) || 1.1,
            parseFloat(container.querySelector('input[name="location.coordinates[1]"]').value) || 1.1
        ];
        const description = container.querySelector('input[name="location.description"]').value || "";
        const day = container.querySelector('input[name="location.day"]').value || "";
...
if (guideRemoveButtons) guideRemoveButtons.forEach((button)=>{
...
if (updateTour) updateTour.addEventListener("click", (e)=>{
    ...
    //if there no guides:
    if (guidesValue === "") formData.delete("guides"); // Use the name of the input field, not its value
    ...
    function getGuideIds() {
        const guideIds = [];
        document.querySelectorAll(".overview-box__detail .remove").forEach((button)=>{
    ...
    document.querySelectorAll(".locations-sub-container").forEach((container, index)=>{
        const coordinates = [
            parseFloat(container.querySelector('input[name="location.coordinates[0]"]').value) || 0,
            parseFloat(container.querySelector('input[name="location.coordinates[1]"]').value) || 0
        ];
        const description = container.querySelector('input[name="location.description"]').value || "";
        const day = Number(container.querySelector('input[name="location.day"]').value || 0);
        formData.append(`locations[${index}][coordinates][0]`, coordinates[0]);
        formData.append(`locations[${index}][coordinates][1]`, coordinates[1]);
        formData.append(`locations[${index}][description]`, description);
        formData.append(`locations[${index}][day]`, day);
...
if (singupFormAdmin) singupFormAdmin.addEventListener("submit", (e)=>{
    e.preventDefault();
    const formData = new FormData();
    ...
    //3) Send the data to funciton that will proccess the post operation
    (0, _administartion.singupAdmin)(formData);
});
if (deleteUserAdminBtn) deleteUserAdminBtn.addEventListener("click", (e)=>{
    e.preventDefault();
    const tourId = updateUserDataAdminBtn.getAttribute("data-user");
    (0, _administartion.deleteUserAdmin)(tourId);
});
...
parcelHelpers.export(exports, "sendPhoneNumber", ()=>sendPhoneNumber);
parcelHelpers.export(exports, "resendCode", ()=>resendCode);
parcelHelpers.export(exports, "verifyPhoneText", ()=>verifyPhoneText);
...
const singup = async (name, email, password, passwordConfirm, phoneNumber)=>{
    try {
...
const sendPhoneNumber = async (formdata)=>{
    try {
        const phoneNumber = formdata.get("phoneNumber");
        const email = formdata.get("email");
        const res = await (0, _axiosDefault.default).post("/api/v1/users/login/phone", {
            phoneNumber,
            email
        });
        if (res.data.status === "success") {
...
const resendCode = async (phoneNumber)=>{
    try {
        ...
        // location.reload(true);   // location.reload() => will reload the page |  location.reload(true) => will force a reload the server and not from the broswer
        (0, _alerts.showAlert)("success", "The code was sended to your phone ");
    } catch (error) {
        console.log(error.response);
        (0, _alerts.showAlert)("error", error.response.data.message);
    }
};
const verifyPhoneText = async (phoneNumber, code)=>{
    try {
        const res = await (0, _axiosDefault.default).post("/api/v1/users/login/phone/verify", {
            code,
            phoneNumber
        });
        if (res.data.status === "success") location.assign("/");
...
const sendConfirmEmail = async (email)=>{
    try {
        const res = await (0, _axiosDefault.default).post("/api/v1/users/confirmEmail", {
            email
        });
        if (res.data.status === "success") (0, _alerts.showAlert)("success", `The email was send to your email, please confirm the new link! `);
    } catch (error) {
        console.log(error);
...
const bookTour = async (tourId, tourDate)=>{
    try {
...
const deleteReview = async (reviewId)=>{
    try {
        if (confirm("are you sure?!")) {
            const res = await (0, _axiosDefault.default).delete(`/api/v1/reviews/${reviewId}`);
            if (res.data.status === "success") {
                (0, _alerts.showAlert)("success", " Review deleted successfully \uD83D\uDC4D");
                location.reload(true);
            }
        } else return (0, _alerts.showAlert)("error", "The delete of the review was canceled");
    } catch (error) {
        (0, _alerts.showAlert)("error", error.response.data.message);
    }
};
...
parcelHelpers.export(exports, "deleteUserAdmin", ()=>deleteUserAdmin);
var _axios = require("axios");
var _axiosDefault = parcelHelpers.interopDefault(_axios);
var _alerts = require("./alerts");
...
const deleteCurrentTour = async (tourId)=>{
    try {
        ...
        } else return (0, _alerts.showAlert)("error", "The delete of the tour was canceled");
    } catch (error) {
...
const singupAdmin = async (formData)=>{
    try {
        const res = await (0, _axiosDefault.default).post("/api/v1/users", formData, {
            headers: {
            }
        });
        if (res.data.status === "success") {
            (0, _alerts.showAlert)("success", "The user Has successfully uploaded");
...
const deleteUserAdmin = async (tourId)=>{
    try {
        if (confirm("are you sure?!")) {
            const res = await (0, _axiosDefault.default).delete(`/api/v1/users/${tourId}`);
            if (res.data.status === "success") {
                (0, _alerts.showAlert)("success", "The user Has successfully uploaded");
                location.assign("/manage-users");
            }
        } else return (0, _alerts.showAlert)("error", "The delete of the user was canceled");
    } catch (error) {
        (0, _alerts.showAlert)("error", error.response.data.message);
    }
};
